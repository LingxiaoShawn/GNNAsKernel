...
for task in "${tasks[@]}"; do
    ...
    python -m train.counting task $task num_workers 8 model.mini_layers 1 model.embs "(0, 1)" model.hops_dim 0 model.gnn_type GINEConv 
    python -m train.counting task $task num_workers 8 model.mini_layers 1 model.hops_dim 0 model.gnn_type GINEConv         
done
...
for task in "${tasks[@]}"; do
    ...
    python -m train.graph_property task $task num_workers 8 model.mini_layers 1 model.embs "(0, 1)" model.hops_dim 0 model.gnn_type GINEConv 
    python -m train.graph_property task $task num_workers 8 model.mini_layers 1 model.hops_dim 0 model.gnn_type GINEConv         
done
