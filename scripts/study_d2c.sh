...
b=3
for task in "${tasks[@]}"; do
    python -m train.counting task $task num_workers 8 model.mini_layers $a model.num_layers $b model.hops_dim 0 model.gnn_type GINEConv    device $gpu
...
tasks=(0 1 2)
for task in "${tasks[@]}"; do
    python -m train.graph_property task $task num_workers 8 model.mini_layers $a model.num_layers $b model.hops_dim 0 model.gnn_type GINEConv    device $gpu
